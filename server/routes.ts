...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (galleryImages.length > 0) {
          console.log(
            `Adding ${galleryImages.length} gallery images to new project ${project.id}`,
          );
          ...
          if (!hasFeatureImage && galleryImages.length > 0) {
            const firstGalleryImage = await storage.getProjectGallery(
              project.id,
            );
            if (firstGalleryImage.length > 0) {
              await storage.setProjectFeatureImage(
                project.id,
                firstGalleryImage[0].id,
              );
            }
          }
        }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const validatedProjectData = insertProjectSchema
          .partial()
          .parse(projectData);
        ...
        if (galleryImages && Array.isArray(galleryImages)) {
          console.log(
            `[PROJECT UPDATE] Processing ${galleryImages.length} gallery images for project ${id}`,
          );

          // Get existing gallery for comparison
          const existingGallery = await storage.getProjectGallery(id);
          console.log(
            `[PROJECT UPDATE] Existing gallery has ${existingGallery.length} images`,
          );
          ...
          for (let i = 0; i < galleryImages.length; i++) {
            ...
            if (image.id && image.id.toString().startsWith("existing-")) {
              ...
              const imageId = parseInt(
                image.id.toString().replace("existing-", ""),
              );
              ...
              if (existingImage) {
                ...
                console.log(
                  `[PROJECT UPDATE] Found existing image by URL: ${image.imageUrl.substring(0, 30)}...`,
                );
                updatedExistingImages.push({
                  id: existingImage.id,
                  caption: image.caption || "",
                ...
                console.log(
                  `[PROJECT UPDATE] Identified new image: ${image.imageUrl.substring(0, 30)}...`,
                );
                trulyNewImages.push({
                  imageUrl: image.imageUrl,
                  caption: image.caption || "",
          ...
          console.log(
            `[PROJECT UPDATE] Found ${updatedExistingImages.length} existing images to update and ${trulyNewImages.length} new images to add`,
          );

          // Create a set of existing image IDs to keep
          const existingIdsToKeep = new Set(
            updatedExistingImages.map((img) => img.id),
          );
          ...
          console.log(
            `[PROJECT UPDATE] Found ${idsToDelete.length} images to delete`,
          );
          ...
          for (const image of updatedExistingImages) {
            const existingImage = existingGallery.find(
              (img) => img.id === image.id,
            );
            ...
            if (existingImage) {
              ...
              const featureChanged =
                existingImage.isFeature !== image.isFeature;

              // Only update if something has changed
              if (captionChanged || orderChanged || featureChanged) {
                console.log(
                  `[PROJECT UPDATE] Updating image ${image.id} - changes: caption=${captionChanged}, order=${orderChanged}, feature=${featureChanged}`,
                );
                ...
                console.log(
                  `[PROJECT UPDATE] No changes for image ${image.id}, skipping update`,
                );
              }
              ...
              if (image.isFeature) {
                ...
                if (!existingImage.isFeature) {
                  console.log(
                    `[PROJECT UPDATE] Setting image ${image.id} as feature image`,
                  );
          ...
          for (const newImage of trulyNewImages) {
            console.log(
              `[PROJECT UPDATE] Adding new image: ${newImage.imageUrl.substring(0, 50)}...`,
            );
            ...
            if (newImage.isFeature) {
              hasFeatureImage = true;
              console.log(
                `[PROJECT UPDATE] Setting new image ${addedImage.id} as feature image`,
              );
          ...
          if (
            !hasFeatureImage &&
            (updatedExistingImages.length > 0 || trulyNewImages.length > 0)
          ) {
            const updatedGallery = await storage.getProjectGallery(id);

            if (updatedGallery.length > 0) {
              console.log(
                `[PROJECT UPDATE] No feature image set, using first image as feature`,
              );
  ...
  app.get(
    `${apiRouter}/projects/:projectId/gallery`,
    async (req: Request, res: Response) => {
      try {
      ...
      try {
        ...
        console.log(`[DEBUG] Adding gallery image to project ${projectId}:`, {
          imageUrl: galleryData.imageUrl
            ? `${galleryData.imageUrl.substring(0, 30)}...`
            : "null",
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedImage = await storage.updateProjectGalleryImage(
          id,
          imageUpdate,
        );
  ...
  app.put(
    `${apiRouter}/projects/:projectId/gallery/:imageId/set-feature`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const projectId = parseInt(req.params.projectId);
        ...
        if (isNaN(projectId) || isNaN(imageId)) {
          return res
            .status(400)
            .json({ message: "Invalid project ID or image ID" });
        }

        const featureImage = await storage.setProjectFeatureImage(
          projectId,
          imageId,
        );
        ...
        if (!featureImage) {
          return res
            ...
            .json({
              message:
                "Gallery image not found or does not belong to this project",
            });
        }
    ...
    try {
      ...
      try {
        ...
        const publishedPosts = posts.filter((post) => {
          const isPublished =
            String(post.published).toLowerCase() === "t" ||
            String(post.published).toLowerCase() === "true" ||
            post.published === true;
          return isPublished;
        });
        console.log(
          `[DEBUG] Found ${publishedPosts.length} published blog posts`,
        );

        if (posts.length > 0) {
          console.log("[DEBUG] Sample post data:", posts[0]);
          console.log(
            "[DEBUG] Sample post 'published' value:",
            posts[0].published,
            "Type:",
            typeof posts[0].published,
          );
        }
        ...
        console.error(
          "[ERROR] Database error fetching published blog posts:",
          dbError,
        );
  ...
  app.get(
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch all blog post categories" });
      }
    },
  );
  ...
  app.get(
    `${apiRouter}/blog/:postId/categories`,
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch blog post categories" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `[DEBUG] Blog gallery images found: ${galleryImages.length}`,
        );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        console.log(
          `[BLOG GALLERY] Received add gallery image request for blog post:`,
          req.params.postId,
        );
        ...
        if (isNaN(postId)) {
          console.log(
            `[BLOG GALLERY ERROR] Invalid post ID: ${req.params.postId}`,
          );
          return res.status(400).json({ message: "Invalid blog post ID" });
        }
        ...
        if (!post) {
          console.log(
            `[BLOG GALLERY ERROR] Blog post not found with ID: ${postId}`,
          );
        ...
        console.log(
          `[BLOG GALLERY] Successfully added gallery image:`,
          galleryImage,
        );
        ...
        console.error(
          `[BLOG GALLERY ERROR] Failed to add gallery image:`,
          error,
        );
        ...
        if (error instanceof z.ZodError) {
          ...
          return res
            .status(400)
            .json({ message: "Invalid gallery data", errors: error.errors });
        }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedImage = await storage.updateBlogGalleryImage(
          id,
          galleryData,
        );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      ...
      `);
        ...
        const post = posts[0];
        console.log(`[DEBUG] Found blog post by slug:`, {
          id: post.id,
          title: post.title,
        });
        ...
        console.log(
          `[DEBUG] Post relations: ${categories.length} categories, ${tags.length} tags, ${galleryImages.length} gallery images`,
        );
  ...
  app.get(`${apiRouter}/blog/:id`, async (req: Request, res: Response) => {
    try {
      ...
      const post = posts[0];
      console.log(`[DEBUG] Found blog post by ID:`, {
        id: post.id,
        title: post.title,
      });
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const { categoryIds, tagIds, galleryImages, ...blogPostData } =
          postData;
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid blog post data", errors: error.errors });
        }
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (
          blogPostData.category === null ||
          blogPostData.category === undefined
        ) {
          blogPostData.category = "";
        }
        ...
        if (galleryImages && Array.isArray(galleryImages)) {
          console.log(
            `[BLOG UPDATE] Processing ${galleryImages.length} gallery images for blog post ${id}`,
          );

          // Get existing gallery for comparison
          const existingGallery = await storage.getBlogGallery(id);
          console.log(
            `[BLOG UPDATE] Existing gallery has ${existingGallery.length} images`,
          );
          ...
          console.log(
            `[BLOG UPDATE] Found ${imagesToDelete.length} images to delete and ${imagesToAdd.length} images to add`,
          );
          ...
          for (let i = 0; i < galleryImages.length; i++) {
            ...
            if (image.id && existingImagesById.has(image.id)) {
              ...
              if (
                ...
                console.log(
                  `[BLOG UPDATE] Updating image ${image.id} with new caption or order`,
                );
              ...
              image.imageUrl &&
              existingImagesByUrl.has(image.imageUrl)
            ) {
              const existingImage = existingImagesByUrl.get(image.imageUrl);
              const caption = image.caption || null;
              const order = image.order || i;
              ...
              if (
                ...
                console.log(
                  `[BLOG UPDATE] Updating image with URL ${image.imageUrl.substring(0, 30)}... with new caption or order`,
                );
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const validatedServiceData = insertServiceSchema
          .partial()
          .parse(serviceData);
        ...
        if (galleryImages && Array.isArray(galleryImages)) {
          console.log(
            `[SERVICE] Processing ${galleryImages.length} gallery images for service ${id}`,
          );

          // Get existing gallery for comparison
          const existingGallery = await storage.getServiceGallery(id);
          console.log(
            `[SERVICE] Existing gallery has ${existingGallery.length} images`,
          );
          ...
          console.log(
            `[SERVICE] Found ${imagesToDelete.length} images to delete and ${imagesToAdd.length} images to add`,
          );
          ...
          for (let i = 0; i < galleryImages.length; i++) {
            ...
            if (image.id && existingImagesById.has(image.id)) {
              ...
              if (existingImage.alt !== alt || existingImage.order !== order) {
                console.log(
                  `[SERVICE] Updating image ${image.id} with new alt or order`,
                );
              ...
              if (existingImage.alt !== alt || existingImage.order !== order) {
                console.log(
                  `[SERVICE] Updating image with URL ${image.imageUrl.substring(0, 30)}... with new alt or order`,
                );
  ...
  app.get(
    `${apiRouter}/services/:serviceId/gallery`,
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `[DEBUG] Gallery images found: ${JSON.stringify(galleryImages)}`,
        );
  ...
  app.put(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const galleryData = insertServiceGallerySchema
          .partial()
          .parse(req.body);
        const updatedImage = await storage.updateServiceGalleryImage(
          id,
          galleryData,
        );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res.status(201).json({
          message:
            "Thank you for your testimonial! It will be reviewed by our team before being published.",
        });
      } catch (error) {
        if (error instanceof z.ZodError) {
          return res
  ...
  app.get(
    `${apiRouter}/admin/testimonials/pending`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch pending testimonials" });
      }
    },
  );

  app.get(
    `${apiRouter}/admin/testimonials/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/testimonials/:id/approve`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/testimonials/:id/revoke`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);

        const unapprovedTestimonial =
          await storage.revokeTestimonialApproval(id);
        ...
        res
          .status(500)
          .json({ message: "Failed to revoke testimonial approval" });
      }
    },
  );
  ...
  app.post(
    ...
    (req: Request, res: Response) => {
      try {
        ...
        const trackedFile = FileManager.trackPendingFile(
          fileUrlToTrack,
          sessionId,
          filename,
        );
  ...
  app.post(
    ...
    (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `Committing files for session ${sessionId}, file count: ${fileUrls?.length || "all"}`,
        );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          `[routes] Cleaning up files for session ${sessionId || "*"}`,
        );
        ...
        try {
          ...
          console.log(
            `[routes] Found ${projectMainImages.length} project main images to preserve`,
          );
          ...
          for (const project of allProjects) {
            if (project.id) {
              ...
              console.log(
                `[routes] Found ${galleryUrls.length} gallery images for project ID ${project.id} to preserve`,
              );
          ...
          console.log(
            `[routes] Total ${allProjectUrls.length} unique project URLs to preserve from database`,
          );
        } catch (dbError) {
          console.error(
            `[routes] Error getting project URLs to preserve:`,
            dbError,
          );
        }
        ...
        console.log(
          `[routes] Final total of ${finalPreserveUrls.length} unique URLs to preserve during cleanup`,
        );
        ...
        if (fileUrls && Array.isArray(fileUrls) && fileUrls.length > 0) {
          ...
          for (const url of fileUrls) {
            try {
              ...
              const result = await FileManager.cleanupSession(
                sessionId || "*",
                url,
                finalPreserveUrls,
              );
        ...
        console.log(
          `[routes] Cleanup completed: ${deletedFiles.length} files deleted, ${failedFiles.length} failed, ${preservedFiles.length} preserved`,
        );
        ...
        const debugInfo = {
          ...
          deletedFiles:
            deletedFiles.length <= 20
              ? deletedFiles
              : `${deletedFiles.length} files (first 5 shown in deletedSample)`,
          preservedFiles:
            preservedFiles.length <= 20
              ? preservedFiles
              : `${preservedFiles.length} files (first 5 shown in preservedSample)`,
          failedFiles:
            failedFiles.length <= 20
              ? failedFiles
              : `${failedFiles.length} files (first 5 shown in failedSample)`,
        };
        ...
        return res.status(200).json({
          ...
          message: fileUrl
            ? deletedFiles.length > 0
              ...
              : preservedFiles.includes(fileUrl)
                ? `Preserved file: ${fileUrl}`
  ...
  app.delete(
    `${apiRouter}/uploadthing/files/:key`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
  ...
  app.post(
    `${apiRouter}/uploadthing/files/delete-batch`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const { keys } = req.body;
        ...
        if (!keys || !Array.isArray(keys) || keys.length === 0) {
          return res
            .status(400)
            .json({ message: "File keys array is required" });
        }
  ...
  app.get(
    `${apiRouter}/uploadthing/file-categories`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        ...
        for (const post of blogPosts) {
          ...
          console.log(
            `Processing blog post ${post.id} (${post.title}), found ${galleryImages.length} gallery images`,
          );

          // Specific known blog image keys - as a fallback if URL parsing fails
          const knownImageKeys: Record<string, string> = {
            "https://utfs.io/f/PFuaKVnX18hb87YYN0DtOVmxrgZuSC6kLz0KBf3E79JiPYoQ":
              "PFuaKVnX18hb87YYN0DtOVmxrgZuSC6kLz0KBf3E79JiPYoQ",
            "https://utfs.io/f/PFuaKVnX18hbaR5cQ2VOzWLZs1FcYNvXfKu7jG549RraP23T":
              "PFuaKVnX18hbaR5cQ2VOzWLZs1FcYNvXfKu7jG549RraP23T",
            "https://utfs.io/f/PFuaKVnX18hbueH7eiuEMxftyqm0wAQVaTXNU2HCulP3L6FD":
              "PFuaKVnX18hbueH7eiuEMxftyqm0wAQVaTXNU2HCulP3L6FD",
            "https://utfs.io/f/PFuaKVnX18hbDBdytq9XO6QEae3p8rvuWcZ1RqH0ngDSdyYA":
              "PFuaKVnX18hbDBdytq9XO6QEae3p8rvuWcZ1RqH0ngDSdyYA",
          };

          for (const image of galleryImages) {
            console.log(
              `  Processing image ${image.id} with URL: ${image.imageUrl}`,
            );
            ...
            if (!key && knownImageKeys[image.imageUrl]) {
              ...
              console.log(
                `  Using known key for URL ${image.imageUrl}: ${key}`,
              );
            }
            ...
            if (key) {
              ...
              console.log(
                `  WARNING: Failed to extract key from URL: ${image.imageUrl}`,
              );
            }
          }
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        for (const quote of quoteRequests) {
          const attachments = await storage.getQuoteRequestAttachments(
            quote.id,
          );
  ...
  app.post(
    `${apiRouter}/newsletter/subscribe`,
    async (req: Request, res: Response) => {
      try {
        ...
        const existingSubscriber = await storage.getNewsletterSubscriberByEmail(
        ...
        if (existingSubscriber) {
          ...
          const updatedSubscriber =
            await storage.updateNewsletterSubscriberStatus(
  ...
  app.post(
    `${apiRouter}/newsletter/unsubscribe`,
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedSubscriber =
          await storage.updateNewsletterSubscriberStatus(subscriber.id, false);
        ...
        res
          .status(500)
          .json({ message: "Failed to process unsubscribe request" });
      }
    },
  );
  ...
  app.get(
    `${apiRouter}/admin/newsletter/subscribers`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
  ...
  app.delete(
    `${apiRouter}/admin/newsletter/subscribers/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.get(
    `${apiRouter}/site-settings/category/:category`,
    async (req: Request, res: Response) => {
      try {
        ...
        console.error(
          `Error fetching site settings for category ${req.params.category}:`,
          error,
        );
        res.status(500).json({ message: "Failed to fetch site settings" });
      }
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.error(
          `Error fetching site setting with key ${req.params.key}:`,
          error,
        );
      ...
      try {
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({
              message: "Invalid site setting data",
              errors: error.errors,
            });
        }
  ...
  app.put(
    `${apiRouter}/admin/site-settings/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/site-settings/key/:key`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const { key } = req.params;
        ...
        console.error(
          `Error updating site setting with key ${req.params.key}:`,
          error,
        );
        res.status(500).json({ message: "Failed to update site setting" });
      }
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        console.log(
          "[QUOTE REQUEST] Request data:",
          JSON.stringify(sanitizedBody),
        );

        try {
          console.log(
            "[QUOTE REQUEST] Validating input data with extended schema",
          );

          // First validate with the extended schema that includes attachments
          const { attachments, ...quoteData } =
            quoteRequestWithAttachmentsSchema.parse(req.body);
          ...
          console.log(
            `[QUOTE REQUEST] Created quote request with ID: ${quote.id}`,
          );
          ...
          if (
            ...
            console.log(
              `[QUOTE REQUEST] Processing ${attachments.length} attachments`,
            );
            ...
            for (const attachment of attachments) {
              try {
                console.log(
                  `[QUOTE REQUEST] Saving attachment: ${attachment.fileName} (${(attachment.fileSize / 1024).toFixed(2)} KB)`,
                );

                if (!attachment.fileUrl || !attachment.fileKey) {
                  console.error(
                    `[QUOTE REQUEST] Invalid attachment data for ${attachment.fileName}: missing fileUrl or fileKey`,
                  );
                ...
                console.log(
                  `[QUOTE REQUEST] Successfully saved attachment: ${attachment.fileName}`,
                );
              } catch (attachmentError) {
                console.error(
                  `[QUOTE REQUEST] Error saving attachment ${attachment.fileName}:`,
                  attachmentError,
                );
                console.error(
                  `[QUOTE REQUEST] Error details:`,
          ...
          console.log(
            "[QUOTE REQUEST] Successfully completed quote request submission",
          );
          res.status(201).json({
          ...
          if (validationError instanceof z.ZodError) {
            console.error(
              "[QUOTE REQUEST] Validation error:",
  ...
  app.get(
    `${apiRouter}/admin/quote/requests`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
  ...
  app.get(
    `${apiRouter}/admin/quote/requests/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/quote/requests/:id/status`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        ...
        res
          .status(500)
          .json({ message: "Failed to update quote request status" });
      }
    },
  );

  app.put(
    `${apiRouter}/admin/quote/requests/:id/reviewed`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        ...
        res
          .status(500)
          .json({ message: "Failed to mark quote request as reviewed" });
      }
    },
  );
  ...
  app.delete(
    `${apiRouter}/admin/quote/requests/attachments/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.post(
    `${apiRouter}/subcontractors/apply`,
    async (req: Request, res: Response) => {
      try {
        console.log(
          "Subcontractor application submitted:",
          JSON.stringify(req.body, null, 2),
        );
        const subcontractorData = insertSubcontractorSchema.parse(req.body);
        console.log(
          "Parsed subcontractor data:",
          JSON.stringify(subcontractorData, null, 2),
        );
        const subcontractor =
          await storage.createSubcontractor(subcontractorData);
        res.status(201).json({
  ...
  app.get(
    `${apiRouter}/admin/subcontractors`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch subcontractor applications" });
      }
    },
  );

  app.get(
    `${apiRouter}/admin/subcontractors/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        ...
        if (!subcontractor) {
          return res
            .status(404)
            .json({ message: "Subcontractor application not found" });
        }
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch subcontractor application" });
      }
    },
  );

  app.put(
    `${apiRouter}/admin/subcontractors/:id/status`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        ...
        const subcontractor = await storage.updateSubcontractorStatus(
          id,
          status,
        );
        if (!subcontractor) {
        ...
        res
          .status(500)
          .json({ message: "Failed to update subcontractor status" });
      }
    },
  );

  app.put(
    `${apiRouter}/admin/subcontractors/:id/notes`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
        ...
        res
          .status(500)
          .json({ message: "Failed to update subcontractor notes" });
      }
    },
  );
  ...
  app.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete subcontractor application" });
      }
    },
  );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch vendor applications" });
      }
    },
  );

  app.get(
    `${apiRouter}/admin/vendors/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/vendors/:id/status`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/vendors/:id/notes`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.delete(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to delete vendor application" });
      }
    },
  );
  ...
  app.put(
    `${apiRouter}/admin/careers/:id/toggle-active`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/careers/:id/toggle-featured`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.get(
    `${apiRouter}/admin/team-members/:id`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/team-members/:id/toggle-active`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.put(
    `${apiRouter}/admin/team-members/:id/order`,
    isAdmin,
    async (req: Request, res: Response) => {
      try {
        const id = parseInt(req.params.id);
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to fetch category site settings" });
      }
    },
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const existingSetting = await storage.getSiteSettingByKey(
          settingData.key,
        );
        if (existingSetting) {
          return res.status(400).json({
        ...
        if (error instanceof z.ZodError) {
          return res
            .status(400)
            .json({ message: "Invalid setting data", errors: error.errors });
        }
        console.error("Error creating site setting:", error);
        res.status(500).json({ message: "Failed to create site setting" });
      }
