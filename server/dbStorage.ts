...
import {
  ...
  type InsertBlogPost,
  type BlogGallery,
  type InsertBlogGallery,
  type Testimonial,
  type InsertTestimonial,
  type Service,
  type InsertService,
  type ServiceGallery,
  type InsertServiceGallery,
  type Message,
  type InsertMessage,
  ...
  type InsertQuoteRequestAttachment,
  type Subcontractor,
...
export class DBStorage implements IStorage {
  ...
  async addProjectGalleryImage(
    galleryImage: InsertProjectGallery,
  ...
  async updateProjectGalleryImage(
    id: number,
    galleryImageUpdate: Partial<InsertProjectGallery>,
  ): Promise<ProjectGallery | undefined> {
    const result = await db
      .update(projectGallery)
  ...
  private async isImageUsedElsewhere(
    imageUrl: string,
    excludeProjectGalleryId?: number,
    ...
    queries.push(
      db
        .select({ count: count() })
        .from(serviceGallery)
        .where(eq(serviceGallery.imageUrl, imageUrl)),
      db
        .select({ count: count() })
        .from(blogGallery)
  ...
  async deleteProjectGalleryImage(id: number): Promise<boolean> {
    ...
    if (imageToDelete.length > 0) {
      ...
      console.log(
        `[dbStorage] Processing deletion of project gallery image (ID: ${id}, project: ${projectId})`,
      );

      // Check if this image is used elsewhere before deleting it from UploadThing
      const isUsedElsewhere = await this.isImageUsedElsewhere(
        imageUrl,
        id,
        projectId,
      );
      ...
      if (isUsedElsewhere) {
        console.log(
          `[dbStorage] Preserving file ${imageUrl} as it is referenced elsewhere`,
        );
      } else {
        console.log(
          `[dbStorage] Image not used elsewhere, deleting from UploadThing: ${imageUrl}`,
        );
        ...
        if (key) {
          try {
            console.log(
              `[dbStorage] Deleting UploadThing file with key: ${key}`,
            );
            await FileManager.deleteFile(imageUrl);
          } catch (error) {
            console.error(
              `[dbStorage] Error deleting file from UploadThing:`,
              error,
            );
          }
        }
  ...
  async deleteAllProjectGalleryImages(projectId: number): Promise<boolean> {
    ...
    console.log(
      `[dbStorage] Processing deletion of all gallery images for project ${projectId} (${imagesToDelete.length} images)`,
    );
    ...
    for (const image of imagesToDelete) {
      ...
      if (isImageUsedElsewhere) {
        console.log(
          `[dbStorage] Preserving file ${imageUrl} as it's used elsewhere in the system`,
        );
      } else {
        console.log(
          `[dbStorage] File ${imageUrl} is not used elsewhere, deleting from storage`,
        );
        await FileManager.deleteFile(imageUrl);
      }
    }
  ...
  async addBlogGalleryImage(
    galleryImage: InsertBlogGallery,
  ): Promise<BlogGallery> {
    console.log(
      `[DB STORAGE] Attempting to add blog gallery image:`,
      galleryImage,
    );
    try {
      const result = await db
        ...
        .returning();
      console.log(
        `[DB STORAGE] Successfully added blog gallery image:`,
        result[0],
      );
      return result[0];
    } catch (error) {
      console.error(
        `[DB STORAGE ERROR] Failed to add blog gallery image:`,
        error,
      );
      throw error;
    }
  ...
  async updateBlogGalleryImage(
    id: number,
    galleryImageUpdate: Partial<InsertBlogGallery>,
  ): Promise<BlogGallery | undefined> {
    const result = await db
      .update(blogGallery)
      .set(galleryImageUpdate)
  ...
  async deleteBlogGalleryImage(id: number): Promise<boolean> {
    ...
    if (imageToDelete.length > 0) {
      ...
      console.log(
        `[dbStorage] Processing deletion of blog gallery image (ID: ${id}, post: ${postId})`,
      );
  ...
  async deleteAllBlogGalleryImages(postId: number): Promise<boolean> {
    ...
    console.log(
      `[dbStorage] Processing deletion of all gallery images for blog post ${postId} (${imagesToDelete.length} images)`,
    );
    ...
    for (const image of imagesToDelete) {
      ...
      const otherImagesWithSameUrl = await db
        .select()
        .from(blogGallery)
        .where(
          and(
      ...
      const postsWithSameImage = await db
        .select()
        .from(blogPosts)
        .where(and(ne(blogPosts.id, postId), eq(blogPosts.image, imageUrl)));

      const isImageUsedElsewhere =
        otherImagesWithSameUrl.length > 0 || postsWithSameImage.length > 0;
  ...
  async getBlogTags(): Promise<SimpleBlogTag[]> {
    // Use raw SQL to avoid schema mismatch
    const results = await db.execute("SELECT id, name, slug FROM blog_tags");
    return results;
  }
  ...
  async createBlogTag(tag: InsertBlogTag): Promise<SimpleBlogTag> {
    // Use raw SQL to avoid schema mismatch
    await db.execute("INSERT INTO blog_tags (name, slug) VALUES ($1, $2)", [
      tag.name,
      tag.slug,
    ]);
  ...
  async updateBlogPostCategories(
    postId: number,
    categoryIds: number[],
  ): Promise<void> {
  ...
  async revokeTestimonialApproval(
    id: number,
  ): Promise<Testimonial | undefined> {
    const results = await db
      .update(testimonials)
  ...
  async addServiceGalleryImage(
    galleryImage: InsertServiceGallery,
  ...
  async updateServiceGalleryImage(
    id: number,
    galleryImageUpdate: Partial<InsertServiceGallery>,
  ): Promise<ServiceGallery | undefined> {
    const result = await db
  ...
  async deleteServiceGalleryImage(id: number): Promise<boolean> {
    ...
    if (imageToDelete.length > 0) {
      ...
      console.log(
        `[dbStorage] Processing deletion of service gallery image (ID: ${id}, service: ${serviceId})`,
      );
  ...
  async deleteAllServiceGalleryImages(serviceId: number): Promise<boolean> {
    ...
    console.log(
      `[dbStorage] Processing deletion of all gallery images for service ${serviceId} (${imagesToDelete.length} images)`,
    );
    ...
    for (const image of imagesToDelete) {
      ...
      const projectGalleryWithSameUrl = await db
        .select()
        .from(projectGallery)
        .where(eq(projectGallery.imageUrl, imageUrl));

      const blogGalleryWithSameUrl = await db
        .select()
        .from(blogGallery)
        .where(eq(blogGallery.imageUrl, imageUrl));
  ...
  async updateNewsletterSubscriber(
    id: number,
    updates: Partial<InsertNewsletterSubscriber>,
  ): Promise<NewsletterSubscriber | undefined> {
    const [result] = await db
  ...
  async markQuoteRequestAsReviewed(
    id: number,
  ): Promise<QuoteRequest | undefined> {
    const [result] = await db
      .update(quoteRequests)
  ...
  async getQuoteRequestAttachments(
  ...
  async createQuoteRequestAttachment(
    attachment: InsertQuoteRequestAttachment,
  ): Promise<QuoteRequestAttachment> {
    const [result] = await db
  ...
  async deleteAllQuoteRequestAttachments(
    quoteRequestId: number,
  ): Promise<boolean> {
    const result = await db
      .delete(quoteRequestAttachments)
  ...
  async updateSubcontractor(
    id: number,
    updates: Partial<InsertSubcontractor>,
  ...
  async updateSubcontractorStatus(
    id: number,
    status: string,
  ): Promise<Subcontractor | undefined> {
    const [result] = await db
  ...
  async updateSubcontractorNotes(
    id: number,
    notes: string,
  ): Promise<Subcontractor | undefined> {
    const [result] = await db
  ...
  async getFeaturedJobPostings(): Promise<JobPosting[]> {
    return db
      .select()
      .from(jobPostings)
      .where(and(eq(jobPostings.active, true), eq(jobPostings.featured, true)))
      .orderBy(jobPostings.createdAt);
  }
  ...
  async updateJobPosting(
    id: number,
    jobPostingUpdate: Partial<InsertJobPosting>,
  ...
  async updateTeamMember(
    id: number,
    teamMemberUpdate: Partial<InsertTeamMember>,
  ...
  async updateSiteSetting(
    id: number,
    settingUpdate: Partial<InsertSiteSetting>,
